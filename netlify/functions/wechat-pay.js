...
exports.handler = async (event) => {
  try {
    ...
    const { userId, plan = "monthly" } = JSON.parse(event.body || "{}");
    ...
    const total_fee = plan === "yearly" ? 39900 : 5000; // ¥399.00 or ¥50.00
    const out_trade_no = `${Date.now()}-${Math.floor(Math.random() * 1000)}`;

    // Step 2: WeChat Credentials
    ...
    // Step 3: Parameters to sign
    const params = {
      appid,
      mch_id,
      nonce_str: generateNonceStr(),
      body: `Calorie AI ${plan === "yearly" ? "Yearly" : "Monthly"} Plan`,
      out_trade_no,
    ...
    // Step 6: Send request to WeChat Pay
